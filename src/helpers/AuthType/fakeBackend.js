    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (validUser["length"] === 1) {
          ...
          localStorage.setItem("authToken", response.data.token); // Example for setting the token

          // Inside the /post-jwt-login reply
          sessionStorage.setItem('authUser', JSON.stringify({ ...validUser[0], accessToken: token }));
